...
urlpatterns = patterns('app.auth.views',
    ...
    url(r'^confirmation/(?P<username>[A-Za-z0-9\._-]{3,25})/(?P<confirmation>[\w-]+)$', 'validate_user', name='confirmation'),
    url(r'^delete/(?P<username>[A-Za-z0-9\._-]{3,25})/(?P<confirmation>[\w-]+)$', 'delete_user', name='delete'), # delete user with confirmation_hash

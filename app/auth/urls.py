...
urlpatterns = patterns('app.auth.views',
    ...
    url(r'^confirmation/(?P<username>.+)/(?P<confirmation>[\w-]+)$', 'validate_user', name='confirmation'),
    url(r'^delete/(?P<username>.+)/(?P<confirmation>[\w-]+)$', 'delete_user', name='delete'), # delete user with confirmation_hash
